# 1. Getting Started (시작하기)
```
test_electric_scooter.py와 test.mp4 혹은 electric_scooter.zip 저장 
```

# 2. Execution
```
1) test_electric_scooter.py, test.pm4 다운로드
  PM-tracker.pt, test.mp4와 같은 폴더에 저장 후 VS CODE로 실행

2) electric_scooter.zip 다운로드
  압축해제 후 VS CODE로 실행
```

# 3. Caution
```
  프레임 계산을 통한 정확도 산출이기 때문에 test.mp4 영상은 전동킥보드가 촬영된 부분만 잘라서 실행
```

# Test result
```
1) 화질 개선 전 영상 테스트
![Image](https://github.com/user-attachments/assets/ec914016-1b9e-4765-b07a-c1a0b8591821)
**카카오톡 영상 전송 시 화질 저하**

2) 화질 개선 후 영상 테스트(원본 촬영 영상)

![image](https://github.com/user-attachments/assets/94541bec-b240-4b28-a906-4e19c8d1db23)
=> 화질에 따른 정확도 차이 존재
```
